...
- ローカル専用の起動とHost検証、入力長・生成重複の制限を追加。
- .envのDBパス反映、生成全件失敗の状態、同期APIによるイベントループ停止を修正。
- 複数購読者へのSSE再生と重複発言の抑制、再起動時の中断状態回復を追加。
- 会議削除APIを追加。履歴で使用中の代理AI削除は説明付き409を返す。
- 追加質問後のレポート更新と、追加質問のプロンプト重複を修正。
- 依存関係を更新し、Python依存もハッシュ付きで固定。回帰テストを追加。
- README・開発ガイド・データ送信と演出の説明を整備。外部フォント取得を廃止。
